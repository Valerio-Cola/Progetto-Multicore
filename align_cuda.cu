...
__global__ void sequencer(unsigned long *g_seq_length, int *g_pat_number, char *g_sequence, unsigned long *d_pat_length,
 ...
 */
	...
	CUDA_CHECK_FUNCTION(cudaFree(g_seq_length));
	CUDA_CHECK_FUNCTION(cudaFree(g_pat_number));
	CUDA_CHECK_FUNCTION(cudaFree(g_sequence));
